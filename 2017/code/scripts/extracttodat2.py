...
r = urllib.urlopen('http://maxsat.ia.udl.cat/detailed/incomplete-pms-crafted-table.html').read()
...
for row in rows:
	...
	for file in files:
		value = file.string
#		print value
		for root, dirnames, filenames in os.walk('./pms_crafted'):
...
#			print solution.font.contents
			sol = solution.font.contents[0].string.encode('ascii','ignore')
			time = solution.font.contents[2].string.encode('ascii', 'ignore')
#		if len(solution.font.contents) > 1:
